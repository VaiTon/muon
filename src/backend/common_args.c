	...
	bool relativize)
{
	...
	/*
	 * Keep target/dependency link arguments separate while adding the
	 * arguments generated by the backend.  In particular, link_with is
	 * emitted as a linker group, and target link arguments must be inside
	 * that group.  This matters for arguments such as -lgcc: when they are
	 * emitted before the group, the archive can be scanned before the
	 * objects that need its helper functions are seen.
	 */
	obj target_link_args;
	obj_array_dup(wk, tgt->dep_internal.link_args, &target_link_args);
	tgt->dep_internal.link_args = make_obj(wk, obj_array);
	...
	if (have_link_with) {
		...
		obj_array_extend(wk, tgt->dep_internal.link_args, target_link_args);
