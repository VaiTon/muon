	...
	*found = false;
	...
	obj compile_args = make_obj(wk, obj_array);
	{
		...
		if (run_cmd_argv(wk, &cmd_ctx, (char *const *)argv, NULL, 0) && cmd_ctx.status == 0) {
			...
			const struct str line = { .s = cmd_ctx.out.buf, .len = cmd_ctx.out.len };
			obj tokens = str_split(wk, &line, NULL);
			obj_array_extend(wk, compile_args, tokens);
		}
		run_cmd_ctx_destroy(&cmd_ctx);
	}
	...
	obj link_args = make_obj(wk, obj_array);
	{
		...
		if (run_cmd_argv(wk, &cmd_ctx, (char *const *)argv, NULL, 0) && cmd_ctx.status == 0) {
			...
			const struct str line = { .s = cmd_ctx.out.buf, .len = cmd_ctx.out.len };
			obj tokens = str_split(wk, &line, NULL);
			obj_array_extend(wk, link_args, tokens);
		}
		run_cmd_ctx_destroy(&cmd_ctx);
	}
